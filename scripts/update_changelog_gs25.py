"""
SPEC-GS25-001: GS25 주간 변경 다이제스트 생성 스크립트
git diff HEAD~1 -- convenience/gs25/_latest.csv 를 분석하여 CHANGELOG.md에 항목을 추가한다.

이 파일은 scripts/update_changelog.py 의 사본이며, GS25 전용 경로·컬럼 규칙만 수정했다.
두 스크립트의 공통 로직 추출은 후속 SPEC-COMMON-LIB-001에서 처리한다.
"""
...
# gs25 _latest.csv 컬럼 순서 (31개)
# code(0), title(1), ..., last_seen_at(29), current_month_file(30)
_COL_CODE = 0
_COL_TITLE = 1
_COL_LAST_SEEN = 29
_COL_MONTH_FILE = 30
...
def get_diff_lines(repo_root: Path) -> list:
    """
    git diff HEAD~1 -- convenience/gs25/_latest.csv 결과를 줄 단위로 반환한다.
    HEAD~1이 없으면 (첫 커밋) 빈 리스트를 반환한다.
    """
    try:
        result = subprocess.run(
            ["git", "diff", "HEAD~1", "--", "convenience/gs25/_latest.csv"],
            capture_output=True,
            text=True,
            cwd=repo_root,
            check=True,
...
def main() -> int:
    ...
    changelog_path = repo_root / "convenience" / "gs25" / "CHANGELOG.md"
